...
function ScoreRing({ score, size = 140 }: { score: number; size?: number }) {
  ...
  useEffect(() => {
    ...
    const c1 = animate(strokeOffset, targetOffset, { duration: 1.8, delay: 0.3, ease: [0.22, 1, 0.36, 1] })
    const c2 = animate(displayScore, score, { duration: 1.8, delay: 0.3, ease: [0.22, 1, 0.36, 1] })
    return () => { c1.stop(); c2.stop() }
  }, [inView, score, circumference, strokeOffset, displayScore])

  const gradeColor = score >= 65 ? 'oklch(0.72 0.14 177)' : score >= 45 ? 'oklch(0.72 0.16 73)' : 'oklch(0.62 0.22 27)'
  ...
  return (
    <div ref={containerRef} className="relative shrink-0" style={{ width: size, height: size }}>
      <div className="absolute inset-0 rounded-full animate-ring-pulse" style={{ boxShadow: `0 0 40px ${gradeColor}40, 0 0 80px ${gradeColor}20` }} />
...
function ImpactBadge({ impact }: { impact: 'high' | 'medium' | 'low' }) {
  return <span className={`impact-${impact} text-xs px-2 py-0.5 rounded-md font-medium`}>{impact.charAt(0).toUpperCase() + impact.slice(1)} impact</span>
}
...
/* ─── Email gate ───────────────────────────────────────────────────────────── */
function EmailGate({ result, onUnlock }: { result: ScanResult; onUnlock: () => void }) {
    ...
    try {
      await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          city: result.city,
          service: result.primaryService,
          overallScore: result.overallScore,
          geminiVisibilityRate: result.geminiVisibility?.visibilityRate,
        }),
      })
      if (typeof window !== 'undefined') {
        localStorage.setItem(`visiblyai_unlocked_${result.id}`, 'true')
      }
      onUnlock()
    } catch {
  ...
  return (
    <Section delay={0.1}>
      <div className="rounded-2xl border border-primary/30 bg-card p-8 md:p-10 text-center relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" aria-hidden>
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[200px] rounded-full"
            style={{ background: 'radial-gradient(ellipse, oklch(0.72 0.14 177 / 10%) 0%, transparent 70%)' }} />
        </div>
        <div className="relative">
          ...
          <h3 className="font-heading font-bold text-xl mb-2">Your full report is ready</h3>
          <p className="text-sm text-muted-foreground max-w-md mx-auto mb-6 leading-relaxed">
            Enter your email to unlock your complete AI visibility analysis — all {result.problems.length} issues, {result.recommendations.length} recommendations, and your Google AI competitor comparison.
          </p>
          ...
          {error && <p className="text-xs severity-critical mt-3">{error}</p>}
          <p className="text-xs text-muted-foreground mt-4">No spam. We&apos;ll send your report and nothing else.</p>
        </div>
      </div>
    </Section>
  )
...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    const raw = typeof window !== 'undefined' ? sessionStorage.getItem(`scan_${id}`) : null
    if (raw) {
      try {
        const parsed = JSON.parse(raw) as ScanResult
        setResult(parsed)
        setUnlocked(localStorage.getItem(`visiblyai_unlocked_${id}`) === 'true')
      } catch {
        router.push('/scan')
      }
      setLoading(false)
    ...
    fetch(`/api/scans/${id}`)
      ...
      .then((data: ScanResult | null) => {
        if (data?.id) {
          ...
          setResult(data)
          setUnlocked(localStorage.getItem(`visiblyai_unlocked_${data.id}`) === 'true')
        } else {
          router.push('/scan')
        }
      })
  ...
  const {
    ...
    recommendations, quickWins, competitorComparison, geminiVisibility,
  } = result

  function toggleQuickWin(id: string) {
    setQuickWinsDone(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  }
  ...
  const createdDate = new Date(result.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })
  const userRank = competitorComparison.findIndex(c => c.isUser) + 1
  const scoreLabel = overallScore >= 80 ? 'Excellent' : overallScore >= 65 ? 'Good' : overallScore >= 50 ? 'Fair' : overallScore >= 35 ? 'Poor' : 'Critical'
  const scoreDesc = overallScore < 50
    ? `AI platforms currently have insufficient signals to recommend ${businessName} for ${primaryService} searches.`
    : `Solid foundation — targeted improvements in the gaps below could significantly increase your AI citation rate.`

  // Free preview: 2 insights. Rest gated.
  const freeInsights = insights.slice(0, 2)
  const gatedInsights = insights.slice(2)

  // Gemini methodology note for competitor section
  const competitorNote = geminiVisibility && !geminiVisibility.error
    ? `Scores show appearance frequency in ${geminiVisibility.totalQuestions} Google AI queries (with live web search grounding). Measured ${new Date(result.createdAt).toLocaleDateString('en-IN')}.`
    : 'Live Google AI query results unavailable for this scan.'
    ...
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-5">
          <div>
            <div className="flex items-center gap-2.5 mb-3 flex-wrap">
              <span className="badge-local text-xs px-3 py-1 rounded-full font-semibold">VisiblyAI Trust Report</span>
              <span className="text-xs text-muted-foreground">{createdDate}</span>
              {result.scanMethod === 'real' && (
                <span className="text-xs px-2 py-0.5 rounded bg-primary/10 text-primary font-medium">Live scan</span>
              )}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div className="rounded-2xl border border-border bg-card shadow-card-md p-6 flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="text-center sm:text-left">
              ...
              <h2 className="font-heading font-black text-lg leading-tight mb-2">{scoreLabel}: {scoreDesc.split('.')[0]}.</h2>
              <p className="text-xs text-muted-foreground leading-relaxed">{scoreDesc}</p>
              {geminiVisibility && !geminiVisibility.error && (
                <div className="mt-3 pt-3 border-t border-border">
                  <p className="text-xs font-semibold text-primary">
                    Google AI visibility: {geminiVisibility.appearedInCount}/{geminiVisibility.totalQuestions} queries
                  </p>
                  <p className="text-xs text-muted-foreground mt-0.5">Measured with live web search grounding</p>
                </div>
              )}
            </div>
          </div>
      ...
      <Section delay={0.08}>
        <SectionLabel
          ...
          description={unlocked ? undefined : `Showing 2 of ${insights.length} insights. Unlock the full report below.`}
        />
        <motion.div
        ...
        >
          {freeInsights.map((insight) => (
            <motion.div key={insight.id} variants={cardReveal}
      ...
      {!unlocked ? (
        ...
        <>
          ...
          {competitorComparison.length > 1 && (
            <Section delay={0.08}>
              <div className="rounded-2xl border border-border bg-card shadow-card-md p-6">
                <div className="flex items-center justify-between mb-4">
                  <SectionLabel eyebrow="Google AI Visibility" title="Competitor Comparison" />
                ...
                <p className="text-xs text-muted-foreground mb-5 leading-relaxed">{competitorNote}</p>
                <motion.div className="space-y-3" initial="hidden" whileInView="visible"
                  viewport={{ once: true, margin: '-40px' }} variants={stagger}>
                  {competitorComparison.map((comp, i) => {
                    const isUser = comp.isUser
                    return (
                      <motion.div key={comp.name + i} variants={cardReveal}
                        className={`flex items-center gap-4 rounded-xl p-4 border transition-all ${
                          isUser ? 'border-primary/30 bg-primary/5 dark:bg-primary/8' : 'border-border bg-secondary/30 dark:bg-secondary/20'
                        }`}>
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                          isUser ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground'
                        }`}>{i + 1}</div>
                        <div className={`w-40 text-sm font-medium truncate shrink-0 ${isUser ? 'text-primary' : 'text-foreground'}`}>
                          {comp.name}{isUser && <span className="text-xs text-muted-foreground font-normal ml-1">(you)</span>}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {isUser ? (
                            userRank === 1 ? <TrendingUp className="w-3.5 h-3.5 text-primary" /> : <TrendingDown className="w-3.5 h-3.5 severity-warning" />
                ...
                {userRank > 1 && (
                  <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      <span className="severity-warning font-semibold">Other businesses appear more often in Google AI results. </span>
                      They are capturing customers who use AI to find {primaryService} in {city}.
                    </p>
                  </motion.div>
                )}
              </div>
          ...
          <Section delay={0.08}>
            ...
            <motion.div className="space-y-3" initial="hidden" whileInView="visible"
              ...
              {problems.map((problem: Problem) => (
                <motion.div key={problem.id} variants={cardReveal}
          ...
          <Section delay={0.08}>
            ...
            <motion.div className="space-y-4" initial="hidden" whileInView="visible"
              viewport={{ once: true, margin: '-40px' }} variants={stagger}>
              {recommendations.map((rec: Recommendation, i: number) => (
                <motion.div key={rec.id} variants={cardReveal}
          ...
          {/* Quick wins */}
          <Section delay={0.08}>
            <SectionLabel eyebrow="Start Today" title="Quick Wins" description="High-impact improvements you can make right now." />
                ...
                {quickWins.map((win: QuickWin, idx: number) => {
                  ...
                  return (
                    <motion.div key={win.id} layout
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`text-sm font-medium mb-0.5 transition-all ${done ? 'line-through text-muted-foreground' : ''}`}>{win.title}</div>
        ...
        <div className="rounded-2xl border border-primary/20 bg-card shadow-card-lg p-8 md:p-10 text-center relative overflow-hidden">
          ...
          <div className="relative">
            <motion.div initial={{ scale: 0.7, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }}
              viewport={{ once: true }} transition={{ duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }}
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }}
              ...
              <motion.p variants={fadeUp} className="text-sm text-muted-foreground max-w-lg mx-auto mb-8 leading-relaxed">
                Get a full AI Visibility Audit by the OptiScale Advisors team. We&apos;ll run extended real AI queries,
                benchmark competitor citation authority, and deliver a complete 90-day action plan to maximise your
                AI recommendation rate.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Button asChild size="lg" className="h-12 px-8 font-semibold shadow-primary gap-2"
                  style={{ background: 'linear-gradient(135deg, oklch(0.62 0.16 177) 0%, oklch(0.52 0.18 200) 100%)', color: 'oklch(1 0 0)' }}>
